...
- The **agent** sends a *functional preference profile* — parameters, never diagnoses.
...
1. No field may encode a diagnosis, condition or identity. Validation rejects unknown
   keys and wrong types; numeric values are clamped into range with a warning.
...
`get_adaptation_capabilities` returns the site name, page id and every supported
capability as `{ key, domain, supported_values }` — semantic dotted keys
(`visual.text_scale`), **never CSS selectors**. Pages cannot add hidden requirements;
unsupported domains are listed explicitly.
...
`measure_rendered_ui` measures the **rendered** page: smallest text, price text sizes,
smallest click target, minimum gap between adjacent controls, minimum contrast (sampled),
count of visible primary actions, running animations, horizontal overflow, occluded
focusables. `verify_profile_fit` compares requested keys against those measurements and
...
- Diagnosis-free by construction: schema, validation and a runtime scanner (see
  `privacy.ts`) refuse diagnosis-like terms.
- Session-only state; no storage, no network for adaptation logic, no profile values in
  URLs or share links.
- `export_adaptation_receipt` returns a compact, portable, diagnosis-free summary
  (`privacy.contains_diagnoses: false, storage: "none", scope: "session"`).
...
The same contract is implemented by two visibly different demos in this repository:
`/shop` (commerce: filters, comparison table, coupons, cart) and `/services`
(administration: multi-step form, deadlines, request statuses, appointments). Identical
profiles produce equivalent, page-appropriate transformations on both — see
